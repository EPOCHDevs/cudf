...
struct check_datetime_format {
   ...
   */
  __device__ std::optional<timestamp_components> check_string(string_view const& d_string)
  {
    ...
    for (auto item : d_format_items) {
      ...
      if (item.item_type == format_char_type::literal) {
        ...
        if (*ptr != item.value) return std::nullopt;
        ptr += item.length;
        length -= item.length;
        continue;
      }
...
struct datetime_formatter_fn {
  ...
  __device__ size_type compute_output_size(T const tstamp) const
  {
    ...
    std::optional<cuda::std::chrono::sys_days> days;
