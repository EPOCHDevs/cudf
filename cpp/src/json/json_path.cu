...
struct json_output {
  ...
  char* output;
  std::optional<size_t> output_len;
 ...
 */
std::pair<std::optional<rmm::device_uvector<path_operator>>, int> build_command_buffer(
  cudf::string_scalar const& json_path, rmm::cuda_stream_view stream)
{
  ...
  return is_empty ? std::pair(std::nullopt, 0)
                  : std::pair(std::make_optional(cudf::detail::make_device_uvector_sync(
...
__launch_bounds__(block_size) CUDF_KERNEL
  void get_json_object_kernel(column_device_view col,
                              ...
                              std::optional<char*> out_buf,
                              std::optional<bitmask_type*> out_validity,
                              std::optional<size_type*> out_valid_count,
                              get_json_object_options options)
{
