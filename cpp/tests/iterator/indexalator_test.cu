...
TYPED_TEST(IndexalatorTest, optional_iterator)
{
  ...
  auto expected_values = thrust::host_vector<std::optional<cudf::size_type>>(host_values.size());
  ...
  std::transform(host_values.begin(),
                 ...
                 expected_values.begin(),
                 [](T v, bool b) {
                   return (b) ? std::make_optional(static_cast<cudf::size_type>(v)) : std::nullopt;
                 });
