...
struct transformer_optional_meanvar {
  using ResultType = std::optional<cudf::meanvar<ElementType>>;

  CUDF_HOST_DEVICE inline ResultType operator()(std::optional<ElementType> const& optional)
  {
...
struct optional_to_meanvar {
  CUDF_HOST_DEVICE inline T operator()(std::optional<T> const& v) { return v.value_or(T{0}); }
};
