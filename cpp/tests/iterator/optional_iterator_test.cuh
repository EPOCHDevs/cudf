...
void nonull_optional_iterator(IteratorTest<T>& testFixture)
{
  ...
  thrust::host_vector<std::optional<T>> replaced_array(host_values.size());
  std::transform(host_values.begin(), host_values.end(), replaced_array.begin(), [](auto s) {
    return std::optional<T>{s};
  });
...
void null_optional_iterator(IteratorTest<T>& testFixture)
{
  ...
  // calculate the expected value by CPU.
  thrust::host_vector<std::optional<T>> optional_values(host_values.size());
  std::transform(host_values.begin(),
                 host_values.end(),
                 host_bools.begin(),
                 optional_values.begin(),
                 [](auto s, bool b) { return b ? std::optional<T>{s} : std::optional<T>{}; });

  thrust::host_vector<std::optional<T>> value_all_valid(host_values.size());
  std::transform(host_values.begin(),
                 host_values.end(),
                 host_bools.begin(),
                 value_all_valid.begin(),
                 [](auto s, bool b) { return std::optional<T>{s}; });
